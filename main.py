...
def is_fleet_in_transit(my_fleet, prospective_x, prospective_y, my_planets):
    for f in my_fleet:
        p = find_planet(my_planets, f.from_planet_id)

        if p == None:
            continue

        tx = p.radius * math.cos(f.angle)
        ty = p.radius * math.sin(f.angle)
        return prospective_x == tx and prospective_y == ty
...
def cal_intercept_with_sun(dx, dy, angle):
    miss_distance = abs(dx * math.sin(angle) - dy * math.cos(angle))
    dot_product = dot_product = dx * math.cos(angle) + dy * math.sin(angle)
    return miss_distance, dot_product
...
def get_inputs(obs):
    ...
    return my_planets, targets, initial_targets, fleets_owned, other_fleets
...
def agent(obs):
    ...
    moves = []
    my_planets, targets, initial_targets, fleets_owned, other_fleets = get_inputs(obs)
    ...
    for mine in my_planets:
        ...
        for nearest in next_nearest_list:
            ...
            if mine.ships >= ships_needed:
                ...
                travel_time = min_dist / fleet_speed

                dx = nearest.x - mine.x
                dy = nearest.y - mine.y

                angle = get_angle(dx, dy, initial_nearest, nearest, obs.angular_velocity, mine, min_dist, fleet_speed)
                if not is_fleet_in_transit(fleets_owned, dx, dy, my_planets):
                    distance_r, dot_product = cal_intercept_with_sun(dx, dy, angle)

                    move = [mine.id, angle, ships_needed]
                    if dot_product > 0:
                        moves.append(move)
                    elif distance_r > sun_config[2]: 
                        moves.append(move)
                break
    return moves
